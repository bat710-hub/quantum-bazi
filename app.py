...
if not api_key:
    ...
    1. **构建初始状态矩阵 (Initial State Matrix $\rho_0$)**:
       - 将年、月、日、时四柱定义为基向量。
       - 建立一个 $5 \times 5$ 的能量张量矩阵，其中矩阵元素 $A_{{ij}}$ 代表五行元素之间的纠缠强度（生克比率）。
       - 展示该系统的轨迹迹 (Trace) 值，以判定整体能量守恒状态。
    
    2. **干支算子相干性演算 (Operator Coherence)**:
       - 将“合”定义为**对称算子**，将“冲”定义为**反对称算子**。
       - 使用对易关系 $[A, B] = AB - BA$ 演算干支间的相互作用。若算子不对易，则代表该维度存在海森堡式的不确定性波动。
    
    3. **时空演化算子 (Evolution Operator $U(t)$)**:
       - 引用施莱格方程逻辑：$i \hbar \frac{{\partial}}{{\partial t}} |\Psi\rangle = \hat{{H}} |\Psi\rangle$。
       - 将当前流年定义为扰动算子 $\hat{{H}}_{{int}}$。
       - 演算初始矩阵在流年扰动下的基向偏移，输出推导后的概率分布矩阵。
    
    4. **坍缩诊断与干预建议**:
       - 计算系统的冯·诺依曼熵 (Von Neumann Entropy) $S = -Tr(\rho \ln \rho)$。熵值过高代表命局混乱，需寻找熵减算子。
       - 给出具体的“相干干预”方案：通过引入特定频率的行为（观测算子）来重新对齐系统矩阵。

    # 输出协议 (必须详细展开)
    1. 必须包含模拟的【推导矩阵】展示（使用文本块呈现数学矩阵结构）。
    2. 逻辑严谨，严禁情绪化，严禁使用非物理学话术。
    3. 直接给出系统坍缩后的最优能级状态。
    4. **格局基态分析**：解析日元强弱、月令提纲，精准定格。
    5. **能量相干性检测**：分析干支冲合刑害对波函数干涉的影响。
    6. **时间箭头与概率分布**：推演大运流年趋势，指出波函数坍缩方向。
    7. **客观缺陷诊断**：冷酷指出命理结构失衡点，禁止感性抚慰。
    8. **观测者干预方案**：基于“观测者效应”，提供严谨的行为/认知建议以修正概率分布。
